...
-- == Be Hub UI Enhancements: rounded corners + rainbow border ==
pcall(function()
    local function addRainbow(frame)
        ...
        -- Update any title text that originally showed "GAZE"
        pcall(function()
            for _, c in ipairs(frame:GetDescendants()) do
                if c:IsA("TextLabel") or c:IsA("TextButton") then
                    if type(c.Text) == "string" and c.Text:match("[Gg][Aa][Zz][Ee]") then
                        c.Text = "Be Hub"
                    end
                end
            end
    ...
    -- try adding to the main frame variable if it exists
    pcall(function()
        if typeof(frame) ~= "Instance" then return end
        addRainbow(frame)
    end)
end)
-- == End of Be Hub UI Enhancements ==
...
local BeHubGui = cloneref(game:GetService("CoreGui")) or game:GetService("CoreGui") or game.Players.LocalPlayer:WaitForChild("PlayerGui")
...
local guiName = "BeHubVerificator"

if BeHubGui:FindFirstChild(guiName) then
...
local beHubLabel = Instance.new("TextLabel")
beHubLabel.Name = "BeHubLabel"
beHubLabel.Text = "Be Hub"
beHubLabel.Font = Enum.Font.SourceSansBold
beHubLabel.TextScaled = true 
beHubLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
beHubLabel.BackgroundTransparency = 1
beHubLabel.Size = labelSize
beHubLabel.Position = UDim2.new(0, 0, 0, 0)
beHubLabel.Parent = frame
...
local function handleDoubleClick()
    if isSmall then
        ...
        beHubLabel.Size = UDim2.new(1, 0, 1, 0)
        task.wait(0.2)
        tween:Play()
